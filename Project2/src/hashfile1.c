...
HT_info* HT_OpenIndex(const char *fileName){
    ...
    HT_info *toReturn = malloc(sizeof(HT_info));
  
    HT_info *temp = (HT_info *) block;

    toReturn->fileDesc = temp->fileDesc;
    toReturn->attrLength = temp->attrLength;
    toReturn->attrType = temp->attrType;
    strcpy(toReturn->attrName, temp->attrName);
    toReturn->numBuckets = temp->numBuckets;
...
int  HT_InsertEntry(HT_info inf, Record record) {
  ...
  // printf("to insert record %d fd = %d\n", record.id, inf.fileDesc);
  ...
  if(staticHash[hashValue] == -1) {
    ...
    memcpy(block, staticHash, sizeof(int) * totalBuckets);
...
int HashStatistics(char *filename) 
{
  ...
  int *bucketRecords;
  int *blocksPerBuckets;
  int *overflowBlocks;
  int totalBlocksUsed = 2; // i count 1st and 2cond block also
  ...
  memcpy(&sinfo, block, sizeof(SHT_info));

  if(strcmp(filename, sinfo.fileName) == 0) {
    // SHT statistics
    if(BF_ReadBlock(fd,1, &block) < 0) {
      perror("BF_ReadBlock");
      return -1;
    }
    // get hash table from block 0
    int *hashTable = (int *) block;

    blocksPerBuckets = malloc(sizeof(int) * sinfo.numBuckets);
    bucketRecords = malloc(sizeof(int) * sinfo.numBuckets);
    overflowBlocks = malloc(sizeof(int) * sinfo.numBuckets);

    for(int i = 0; i < sinfo.numBuckets; i++)
    {
      bucketRecords[i] = blocksPerBuckets[i] = overflowBlocks[i] = 0;
    }
    ...
    for(int i = 0; i < sinfo.numBuckets; i++) {
      ...
      while(nextBlock != -1) {
        ...
        // increase records
        bucketRecords[i] += sht_block->nRecords;
      ...
      printf("\n  Bucket-%d stats:\n    %d records",  i, bucketRecords[i]);
      printf("\n");
    }

    int maxRecords = 0, minRecords = 0, sumRecords = 0;
    maxRecords = minRecords = bucketRecords[0];
    for(int i = 0; i < sinfo.numBuckets; i++) {
      if(bucketRecords[i] > maxRecords) 
        maxRecords = bucketRecords[i];

      if(bucketRecords[i] < minRecords)
        minRecords = bucketRecords[i];

      sumRecords += bucketRecords[i];
    }

    int avgRecords = sumRecords / sinfo.numBuckets;
    ...
    printf("\n  Average number of blocks : \"%d\"", (sumBlocks / sinfo.numBuckets) );
    printf("\n  Buckets with overflow blocks \"%d\"\n  Total overflow blocks \"%d\"", 
      overflowBuckets, totalOverflowBlocks - sinfo.numBuckets);
